...
SITE_DOMAIN = "desk.enjoy-onepage.com"
SITE_NAME = "데스크 기어랩"
...
- 3초 요약 박스 (<div class="summary-box">): 1위 국민템, 2위 가성비 픽, 3위 프리미엄 픽 핵심 한 줄 정의
- 구매 전 필수 체크 기준 3가지 (※ 주의: 공식 상세페이지에 없는 가짜 공학 수치(dB, W, mmHg, mm 등)를 절대 날조하지 말고, 소재의 특성, 안전 인증 유무, 실사용자 체감 기준 등 검증 가능한 팩트로 서술할 것)
- 제품별 상세 분석 (각 모델마다 실제 사용자가 공감할 '👍 장점 2가지'와 타협해야 할 '⚠️ 솔직한 단점 1가지' 명시)
- 핵심 스펙 6열 비교 마크다운 테이블 [포지셔닝 | 상품명 | 가격대 | 주요특징 | 추천대상 | 배송/보증]
...
"""
    ...
    final_rewrite_prompt = f"""당신은 광고나 과장 없이 팩트만을 전달하는 정직한 전문 쇼핑 큐레이터입니다.
작성자의 가짜 신분이나 사칭을 일체 쓰지 말고, 소비자의 입장에서 합리적인 구매 선택을 돕는 정직한 리뷰를 작성하세요.
[절대 엄금]: 공식 상품 정보에 표기되지 않은 임의의 가짜 수치나 인증을 날조하지 마세요.
